...
export default function Home() {
  ...
  const loadNews = async (mode: "initial" | "refresh" = "initial") => {
    ...
    try {
      ...
      if (!response.ok) throw new Error("Unable to load live news")
      ...
      if (mode === "initial") setNewsLoading(false)
      else setNewsRefreshing(false)
    }
  }

  useEffect(() => {
    void loadNews("initial")
    const intervalId = window.setInterval(() => { void loadNews("refresh") }, 5 * 60 * 1000)
  ...
  return (
    <div className="flex flex-col">
      ...
      <section className="relative overflow-hidden bg-white py-10 md:py-14">
        <div className="absolute inset-0 bg-gradient-to-br from-[#0B3D91]/5 to-transparent" />
          ...
          <div className="grid items-center gap-14 md:grid-cols-2 lg:gap-10">
              ...
              <div className="flex flex-col space-y-4 sm:flex-row sm:space-x-4 sm:space-y-0">
                <Link href="/tenders">
                  <Button size="lg" className="w-full sm:w-auto font-semibold">
                    Explore Live Tenders <ArrowRight className="ml-2 h-4 w-4" />
                  </Button>
                </Link>
                <Link href="/ledger">
                  <Button size="lg" variant="outline" className="w-full sm:w-auto font-semibold">Verify a Tender</Button>
                </Link>
              </div>
            ...
            <div className="relative mx-auto w-full max-w-lg lg:max-w-none">
              <div className="relative aspect-square w-full rounded-full border border-gray-100 bg-gray-50/50 p-8 shadow-2xl flex items-center justify-center">
                <div className="absolute inset-0 rounded-full border border-dashed border-[#0B3D91]/20 animate-spin-slow" style={{ animationDuration: '30s' }} />
                <div className="relative z-10 grid grid-cols-2 gap-8">
                  <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="rounded-xl bg-white p-6 shadow-lg flex flex-col items-center text-center">
                    <FileLock className="h-10 w-10 text-[#0B3D91] mb-2" />
                    <span className="text-xs font-bold text-gray-700">Encrypted Bids</span>
                  </motion.div>
                  <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="rounded-xl bg-[#0B3D91] p-6 shadow-lg flex flex-col items-center text-center mt-12">
                    <Blocks className="h-10 w-10 text-white mb-2" />
                    <span className="text-xs font-bold text-white">Immutable Ledger</span>
                  </motion.div>
                  <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.6 }} className="rounded-xl bg-white p-6 shadow-lg flex flex-col items-center text-center -mt-8">
                    <CheckCircle2 className="h-10 w-10 text-[#138808] mb-2" />
                    <span className="text-xs font-bold text-gray-700">Smart Contracts</span>
                  </motion.div>
                  <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.8 }} className="rounded-xl bg-white p-6 shadow-lg flex flex-col items-center text-center mt-4">
            ...
            <div className="grid gap-8 md:grid-cols-4 relative z-10">
              {[
                ...
                <div key={i} className="flex flex-col items-center text-center">
                  <div className="mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-white shadow-md border-4 border-[#F8F9FC] transition-colors relative">
                    <item.icon className="h-8 w-8 text-[#0B3D91]" />
                    <div className="absolute -top-2 -right-2 bg-[#FF9933] text-white text-xs font-bold rounded-full w-6 h-6 flex items-center justify-center">{item.step}</div>
                  </div>
      ...
      <section className="bg-[#F8F9FC] py-10 md:py-14">
        <div className="container mx-auto px-4">
          <div className="mb-8 flex items-center justify-between">
            ...
            <Link href="/tenders" className="hidden md:inline-flex text-[#0B3D91] font-semibold hover:underline items-center gap-1">
        ...
        <div className="container mx-auto px-4">
          ...
          {newsLoading ? (
            ...
            <div className="rounded-3xl border border-gray-200 bg-white p-6 shadow-sm">
              ...
              <Button type="button" className="mt-6 bg-[#0B3D91] hover:bg-[#0B3D91]/90" onClick={() => void loadNews("refresh")}>Try Again</Button>
            </div>
          ) : topStory ? (
            <div className="grid gap-6 lg:grid-cols-[1.4fr_0.9fr]">
              <Card className="overflow-hidden border border-gray-200 bg-white shadow-sm">
                <CardHeader className="border-b border-gray-100 bg-gradient-to-br from-[#0B3D91]/5 to-transparent p-6">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    ...
                    <span className="inline-flex items-center rounded-full bg-[#138808]/10 px-3 py-1 text-xs font-semibold text-[#138808]">Live</span>
                  </div>
                ...
                <CardContent className="space-y-5 p-6">
                  <div className="flex flex-wrap items-center gap-3 text-xs text-gray-500">
                    ...
                    {newsRefreshing ? (
                      <span className="inline-flex items-center gap-2 text-[#0B3D91]"><Loader2 className="h-3.5 w-3.5 animate-spin" /> Refreshing</span>
                    ) : null}
                  </div>
                  <a href={topStory.url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 text-sm font-semibold text-[#0B3D91] hover:underline">
              ...
              <div className="space-y-4">
                {supportingStories.length > 0 ? (
                    ...
                    <Card key={`${article.title}-${article.publishedAt}`} className="border border-gray-200 bg-white shadow-sm transition-shadow hover:shadow-md">
                      <CardContent className="p-5">
                        <div className="flex items-start justify-between gap-3">
                          ...
                          <span className="rounded-full bg-gray-100 px-2.5 py-1 text-[11px] font-semibold text-gray-600">News</span>
                        </div>
                        <div className="mt-4 flex items-center justify-between gap-3 text-xs text-gray-500">
                          ...
                          <a href={article.url} target="_blank" rel="noreferrer" className="font-semibold text-[#0B3D91] hover:underline">Open</a>
                        </div>
                      </CardContent>
                    </Card>
                  ))
                  ...
                  <Card className="border border-gray-200 bg-white shadow-sm">
                    <CardContent className="p-5 text-sm text-gray-600">News items are loading. Please check back in a moment.</CardContent>
                  </Card>
                )}
              </div>
            </div>
